...
description: Help a contributor add a new methodological tradition or evaluation framework to theory-forge
---
...
Each tradition has its own quality criteria, its own analytical workflow, and its own failure modes. this skill lets anyone package a methodological tradition into theory-forge so that:
...
The result is a methodology package: rubric + eval skill + registry entry + documentation.
...
Create an evaluation command at `.claude/skills/eval-[methodology_id].md`:
...
**IMPORTANT**: The eval skill should be substantive, not a thin wrapper. The value is in the evidence guidance, the rating level descriptions, and the methodology-specific failure modes. If the contributor is an expert in this tradition, capture their knowledge.
...
2. Or edit the rubric (`rubrics/[methodology_id].json`) and eval skill directly
...
2. **Eval command**: `.claude/skills/eval-[methodology_id].md`
...
5. **Analytical command**: `.claude/skills/[command_name].md`
   - Usage: `/[command_name]`
...
1. **Expert knowledge, not generic criteria.** The whole point of this skill is to capture what someone trained in a specific tradition knows about quality. Generic criteria like "is the methodology clearly described?" are useless — every tradition has specific things that matter.
