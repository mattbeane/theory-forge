---
name: eval-limitations
description: /eval-limitations - Limitations Section Genre Check
---
...
1. Read `state.json` → `eval_results.limitations.frame_[current_frame].latest`
...
3. Write to `eval_results.limitations.frame_[current_frame].latest`:
   ```json
   {
     "timestamp": "[current ISO timestamp]",
     "scores": {
       "boundary_framing": 1|0,
       "word_count_ok": 1|0,
       "no_apologies": 1|0
     },
     "total": X,
     ...
     "output_file": "analysis/quality/LIMITATIONS_EVAL.md"
   }
   ```
4. Update `updated_at` timestamp
5. Log to `DECISION_LOG.md`: "limitations scored [total]/3 — [verdict]"
